...
public abstract partial record Diagnostic
{
    ...
    {
        public override ImmutableArray<SourceLocation> Locations => [Source.GetLocation(Actual.FullSpan)];
    ...
    public sealed record MissingToken(SourceFileView Source, Token? Previous, Token Next, ExpectedSyntax Expected) : Error
    {
        public override ImmutableArray<SourceLocation> Locations
        {
            get
            {
                ...
                if (Previous is null)
                    return [Source.GetLocation(SourceSpan.EmptyBefore(Next.FullSpan))];

                return [Source.GetLocation(SourceSpan.EmptyAfter(Previous.FullSpan))];
            }
        }
    ...
    {
        public override ImmutableArray<SourceLocation> Locations =>
            [.. OffendingOperators.Select(offendingOp => Source.GetLocation(offendingOp.FullSpan))];
