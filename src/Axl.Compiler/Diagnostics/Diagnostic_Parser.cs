...
public abstract partial record Diagnostic
{
    ...
    {
        public override ImmutableArray<SourceLocation> Locations => [Source.GetLocation(Actual.FullRange)];
    ...
    public sealed record MissingToken(SourceFileView Source, Token? Previous, Token Next, ExpectedSyntax Expected) : Error
    {
        public override ImmutableArray<SourceLocation> Locations
        {
            get
            {
                ...
                if (Previous is null)
                    return [Source.GetLocation(SourceRange.EmptyBefore(Next.FullRange))];

                return [Source.GetLocation(SourceRange.EmptyAfter(Previous.FullRange))];
            }
        }
    ...
    {
        public override ImmutableArray<SourceLocation> Locations =>
            [.. OffendingOperators.Select(offendingOp => Source.GetLocation(offendingOp.FullRange))];
